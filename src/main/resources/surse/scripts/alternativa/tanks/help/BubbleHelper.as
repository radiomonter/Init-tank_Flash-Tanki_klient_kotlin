package alternativa.tanks.help
{
   ...
   public class BubbleHelper extends Helper
   {
      ...
      {
         BubbleHelper.defaultTextFormat = _arg_1;
         BubbleHelper.embededFont = _arg_2;
      }
      
      override public function draw(_arg_1:Point) : void
      {
         _size.x = int(_arg_1.x);
         _size.y = int(_arg_1.y);
         this.outline.graphics.clear();
         this.outline.graphics.beginFill(lineColor,lineAlpha);
         this.outline.graphics.drawRoundRect(-lineThickness,-lineThickness,_arg_1.x + lineThickness * 2,_arg_1.y + lineThickness * 2,r + 2,r + 2);
         this.outline.graphics.drawRoundRect(0,0,_arg_1.x,_arg_1.y,r,r);
         this.back.graphics.clear();
         this.back.graphics.beginFill(fillColor,fillAlpha);
         this.back.graphics.drawRoundRect(0,0,_arg_1.x,_arg_1.y,r,r);
         var _local_2:Number = this._arrowLehgth / arrowWidth;
         if(Boolean(this._arrowAlign & HelperAlign.TOP_MASK))
         {
            ...
            this.outlineArrowTarget.y = -_local_2 * (arrowWidth + lineThickness * 2);
         }
         else if(Boolean(this._arrowAlign & HelperAlign.MIDDLE_MASK))
         {
            this.arrowP1.y = _arg_1.y - arrowWidth >> 1;
         ...
         {
            this.arrowP1.y = _arg_1.y;
            this.arrowP2.y = _arg_1.y;
            this.arrowTarget.y = _arg_1.y + this._arrowLehgth;
            this.outlineArrowP1.y = _arg_1.y;
            this.outlineArrowP2.y = _arg_1.y;
            this.outlineArrowTarget.y = _arg_1.y + _local_2 * (arrowWidth + lineThickness * 2);
         }
         if(Boolean(this._arrowAlign & HelperAlign.LEFT_MASK))
         {
            ...
            else
            {
               ...
               this.arrowP1.x = 0;
               this.arrowP2.x = 0;
               this.outlineArrowTarget.x = -_local_2 * (arrowWidth + lineThickness * 2);
         ...
         {
            this.arrowTarget.x = _arg_1.x - arrowWidth >> 1;
         ...
         else
         {
            if(this.arrowDirection == HelperArrowDirection.VERTICAL)
            {
               this.arrowTarget.x = _arg_1.x - arrowShift;
               this.arrowP1.x = this.arrowTarget.x;
            ...
            {
               this.arrowTarget.x = _arg_1.x + this._arrowLehgth;
               this.arrowP1.x = _arg_1.x;
               this.arrowP2.x = _arg_1.x;
               this.outlineArrowTarget.x = _arg_1.x + _local_2 * (arrowWidth + lineThickness * 2);
               this.outlineArrowP1.x = _arg_1.x;
               this.outlineArrowP2.x = _arg_1.x;
            }
            if(Boolean(this._arrowAlign & HelperAlign.TOP_MASK))
            {
               this.outline.graphics.drawRect(this.arrowP1.x,this.arrowP1.y - lineThickness,this.arrowP2.x - this.arrowP1.x,lineThickness);
         ...
         this.bmp.bitmapData.draw(this.backContainer,_local_4,new ColorTransform(),BlendMode.NORMAL,null,true);
         stage.quality = _local_3;
         this.descriptionLabel.width = _arg_1.x - this.margin * 2;
      }
      ...
      protected function doAlign() : void
      {
         ...
         if(parent != null)
         {
            _local_1 = parent.globalToLocal(_targetPoint);
            this.x = Math.round(_local_1.x - this.outlineArrowTarget.x);
            this.y = Math.round(_local_1.y - this.outlineArrowTarget.y);
         }
      }
      ...
      public function set arrowLehgth(_arg_1:int) : void
      {
         this._arrowLehgth = _arg_1;
      }
      ...
      public function set arrowAlign(_arg_1:int) : void
      {
         if(_arg_1 == HelperAlign.MIDDLE_CENTER)
         {
            this._arrowAlign = HelperAlign.BOTTOM_LEFT;
         }
         else
         {
            this._arrowAlign = _arg_1;
         }
      ...
      {
         this.descriptionLabel.htmlText = _arg_1;
