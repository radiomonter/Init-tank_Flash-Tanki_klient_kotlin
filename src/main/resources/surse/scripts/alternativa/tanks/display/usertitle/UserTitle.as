package alternativa.tanks.display.usertitle
{
   import alternativa.engine3d.core.Clipping;
   import alternativa.engine3d.materials.TextureMaterial;
   import alternativa.engine3d.objects.Sprite3D;
   import alternativa.math.Vector3;
   import alternativa.tanks.battle.scene3d.scene3dcontainer.Scene3DContainer;
   import alternativa.tanks.models.inventory.InventoryItemType;
   import alternativa.tanks.utils.MathUtils;
   import controls.Label;
   import controls.base.LabelBase;
   import filters.Filters;
   import flash.display.BitmapData;
   import flash.geom.Matrix;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.geom.Vector3D;
   import flash.text.GridFitType;
   import flash.text.TextFieldAutoSize;
   import flash.utils.getTimer;
   import forms.ColorConstants;
   import forms.ranks.SmallRankIcon;
   import obfuscation.bude.BattleTeam;
   import obfuscation.modo.Johad;
   import obfuscation.mujusaqim.MessageColor;
   import platform.client.fp10.core.type.AutoClosable;
   import platform.client.fp10.core.type.IGameObject;
   
   public class UserTitle implements AutoClosable
   {
      
      private static const matrix:Matrix = new Matrix();
      
      private static const RANK_ICON_SIZE:int = 13;
      
      private static const RANK_ICON_OFFSET_Y:int = 3;
      
      private static const PREMIUM_RANK_ICON_OFFSET_Y:int = 0;
      
      private static const PREMIUM_RANK_ICON_SIZE:int = 18;
      
      private static const LABEL_HEIGHT:int = 13;
      
      private static const RESISTANCE_ICON_WIDTH:int = 11;
      
      private static const RESISTANCE_ICON_SPACING_X:int = 7;
      
      private static const RESISTANCE_MAX_TEXT_WIDTH:int = 10;
      
      private static const EFFECTS_ICON_SIZE:int = 18;
      
      private static const LABEL_SPACING:int = 2;
      
      private static const HEALTH_BAR_SPACING_Y:int = 2;
      
      private static const WEAPON_BAR_SPACING_Y:int = -1;
      
      private static const EFFECTS_SPACING_Y:int = 4;
      
      private static const EFFECTS_SPACING_X:int = 4;
      
      private static const BAR_WIDTH:int = 100;
      
      private static const BAR_HEIGHT:int = 8;
      
      private static const TEXTURE_MARGIN:int = 3;
      
      private static const TEXTURE_MARGIN_2:int = 2 * TEXTURE_MARGIN;
      
      private static const rankIcon:SmallRankIcon = new SmallRankIcon();
      
      private static const inventoryItemTypes:Vector.<int> = Vector.<int>([InventoryItemType.ULTIMATE,Johad.hocypo,Johad.ARMOR,Johad.ryjozywih,Johad.husyb]);
      
      public static const ALPHA_SPEED:Number = 0.002;
      
      public static var showAddition:Boolean = false;
      
      private var configFlags:int;
      
      private var dirtyFlags:int;
      
      private var sprite:Sprite3D;
      
      private var textureRect:Rectangle;
      
      private var label:Label;
      
      private var healthBar:ProgressBar;
      
      private var weaponBar:ProgressBar;
      
      private var effectIndicators:Vector.<EffectIndicator>;
      
      private var numVisibleIndicators:int;
      
      private var effectIndicatorsY:int;
      
      private var rankId:int;
      
      private var labelText:String;
      
      private var health:int;
      
      private var weaponStatus:int;
      
      private var indicatorsNumberChanged:Boolean;
      
      private var teamType:BattleTeam = BattleTeam.NONE;
      
      private var healthBarSkin:ProgressBarSkin = ProgressBarSkin.HEALTHBAR_DM;
      
      private var isSuspicious:Boolean;
      
      private var hidden:Boolean = true;
      
      private var time:int;
      
      private var material:TextureMaterial;
      
      private var texture:BitmapData;
      
      private var zOffset:Number;
      
      private var container:Scene3DContainer;
      
      private var resistanceLabel:Label = new LabelBase();
      
      private var size:Size2D = new Size2D();
      
      private var maxHealth:int;
      
      private var isLocal:Boolean;
      
      private var hasPremium:Boolean;
      
      private var resistance:int;
      
      private var tankObject:IGameObject;
      
      public function UserTitle(_arg_1:Number, _arg_2:Scene3DContainer, _arg_3:int, _arg_4:Boolean, _arg_5:IGameObject = null)
      {
         super();
         this.zOffset = _arg_1;
         this.container = _arg_2;
         this.maxHealth = _arg_3;
         this.isLocal = _arg_4;
         this.tankObject = _arg_5;
         this.material = new TextureMaterial();
         this.material.uploadEveryFrame = true;
         this.sprite = new Sprite3D(100,100,this.material);
         if(_arg_1 == 0)
         {
            this.sprite.depthTest = false;
         }
         this.sprite.clipping = Clipping.FACE_CLIPPING;
         this.sprite.perspectiveScale = false;
         this.sprite.alpha = 0;
         this.sprite.visible = false;
         this.sprite.useShadowMap = false;
         this.sprite.useLight = false;
         if(!_arg_4)
         {
            this.sprite.originY = 1;
         }
         this.hidden = true;
      }
      
      private function updateAdditionalUserTitle(_arg_1:uint) : void
      {
         if(this.isLocal)
         {
         }
      }
      
      public function getTexture() : BitmapData
      {
         return this.texture;
      }
      
      public function hide() : void
      {
         this.hidden = true;
      }
      
      public function show() : void
      {
         this.hidden = false;
      }
      
      public function setConfiguration(_arg_1:int) : void
      {
         if(this.configFlags != _arg_1)
         {
            this.configFlags = _arg_1;
            this.updateConfiguration();
         }
      }
      
      public function setResistance(_arg_1:int) : void
      {
         if(this.resistance != _arg_1)
         {
            this.resistance = _arg_1;
            this.resistanceLabel.text = _arg_1.toString();
            this.invalidateAllConfigFlags();
         }
      }
      
      public function setTeamType(_arg_1:BattleTeam) : void
      {
         var _local_2:ProgressBarSkin = null;
         if(this.teamType != _arg_1)
         {
            this.teamType = _arg_1;
            _local_2 = ProgressBarSkin.getHealthBarSkin(_arg_1);
            if(this.healthBarSkin.color != _local_2.color)
            {
               this.updateAdditionalUserTitle(_local_2.color);
            }
            this.healthBarSkin = _local_2;
            this.invalidateConfigFlags(TitleConfigFlags.LABEL | TitleConfigFlags.HEALTH | TitleConfigFlags.WEAPON);
         }
      }
      
      public function setRank(_arg_1:int) : void
      {
         if(this.rankId != _arg_1)
         {
            this.rankId = _arg_1;
            if(this.hasAnyFlag(TitleConfigFlags.LABEL))
            {
               this.invalidateAllConfigFlags();
            }
         }
      }
      
      private function invalidateAllConfigFlags() : void
      {
         this.invalidateConfigFlags(TitleConfigFlags.LABEL | TitleConfigFlags.HEALTH | TitleConfigFlags.WEAPON | TitleConfigFlags.EFFECTS);
      }
      
      public function setPremium(_arg_1:Boolean) : void
      {
         if(this.hasPremium != _arg_1)
         {
            this.hasPremium = _arg_1;
            if(this.hasAnyFlag(TitleConfigFlags.LABEL))
            {
               this.invalidateAllConfigFlags();
            }
         }
      }
      
      public function setLabelText(_arg_1:String) : void
      {
         if(this.labelText != _arg_1)
         {
            this.labelText = _arg_1;
            if(this.hasAnyFlag(TitleConfigFlags.LABEL))
            {
               this.updateConfiguration();
               this.invalidateAllConfigFlags();
            }
         }
      }
      
      public function setHealth(_arg_1:int) : void
      {
         if(this.health != _arg_1)
         {
            this.health = _arg_1;
            this.invalidateConfigFlags(TitleConfigFlags.HEALTH);
         }
      }
      
      public function setWeaponStatus(_arg_1:int) : void
      {
         if(this.weaponStatus != _arg_1)
         {
            this.weaponStatus = _arg_1;
            this.invalidateConfigFlags(TitleConfigFlags.WEAPON);
         }
      }
      
      public function showIndicator(_arg_1:int, _arg_2:int = -1, _arg_3:Boolean = false, _arg_4:int = 0) : void
      {
         var _local_5:EffectIndicator = null;
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS))
         {
            _local_5 = this.getEffectIndicatorById(_arg_1);
            if(_local_5 != null)
            {
               if(_local_5.isHidden())
               {
                  this.changeVisibleIndicatorsNumber(1);
               }
               _local_5.show(_arg_2,_arg_3,_arg_4);
            }
         }
      }
      
      public function hideIndicator(_arg_1:int, _arg_2:Boolean = false) : void
      {
         var _local_3:EffectIndicator = null;
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS))
         {
            _local_3 = this.getEffectIndicatorById(_arg_1);
            if(_local_3 != null)
            {
               if(_arg_2)
               {
                  _local_3.activeAfterDeath = false;
               }
               _local_3.hide();
            }
         }
      }
      
      public function hideIndicators() : void
      {
         var _local_1:int = 0;
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS) && this.effectIndicators != null)
         {
            for each(_local_1 in inventoryItemTypes)
            {
               this.hideIndicator(_local_1);
            }
         }
      }
      
      internal function doHideIndicator(_arg_1:EffectIndicator) : void
      {
         _arg_1.clear(this.texture);
         this.changeVisibleIndicatorsNumber(-1);
      }
      
      public function update(_arg_1:Vector3) : void
      {
         var _local_2:EffectIndicator = null;
         this.setPosition(_arg_1);
         var _local_3:int = getTimer();
         var _local_4:int = _local_3 - this.time;
         this.time = _local_3;
         this.updateVisibility(_local_4);
         if(this.dirtyFlags != 0)
         {
            if(this.isDirtyAndHasOption(TitleConfigFlags.LABEL))
            {
               this.updateLabel();
            }
            if(this.isDirtyAndHasOption(TitleConfigFlags.HEALTH))
            {
               this.healthBar.setSkin(this.healthBarSkin);
               this.healthBar.progress = this.health;
               this.healthBar.draw(this.texture);
            }
            if(this.isDirtyAndHasOption(TitleConfigFlags.WEAPON))
            {
               this.weaponBar.progress = this.weaponStatus;
               this.weaponBar.draw(this.texture);
            }
            if(this.isDirtyAndHasOption(TitleConfigFlags.EFFECTS))
            {
               for each(_local_2 in this.effectIndicators)
               {
                  _local_2.forceRedraw();
               }
            }
            this.dirtyFlags = 0;
         }
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS))
         {
            this.updateEffectIndicators(_local_3,_local_4);
         }
      }
      
      private function isDirtyAndHasOption(_arg_1:int) : Boolean
      {
         return (_arg_1 & this.dirtyFlags & this.configFlags) == _arg_1;
      }
      
      private function updateEffectIndicators(_arg_1:int, _arg_2:int) : void
      {
         var _local_3:EffectIndicator = null;
         var _local_4:int = 0;
         var _local_5:int = 0;
         var _local_6:int = int(this.effectIndicators.length);
         if(this.indicatorsNumberChanged)
         {
            this.indicatorsNumberChanged = false;
            _local_5 = this.size.width + TEXTURE_MARGIN_2 - this.numVisibleIndicators * EFFECTS_ICON_SIZE - (this.numVisibleIndicators - 1) * EFFECTS_SPACING_X >> 1;
            _local_4 = 0;
            while(_local_4 < _local_6)
            {
               _local_3 = this.effectIndicators[_local_4];
               if(_local_3.isVisible())
               {
                  _local_3.clear(this.texture);
               }
               if(!_local_3.isHidden())
               {
                  _local_3.setPosition(_local_5,this.effectIndicatorsY);
                  _local_5 += EFFECTS_ICON_SIZE + EFFECTS_SPACING_X;
               }
               _local_4++;
            }
         }
         _local_4 = 0;
         while(_local_4 < _local_6)
         {
            _local_3 = this.effectIndicators[_local_4];
            _local_3.update(_arg_1,_arg_2,this.texture);
            _local_4++;
         }
      }
      
      private function changeVisibleIndicatorsNumber(_arg_1:int) : void
      {
         this.numVisibleIndicators += _arg_1;
         this.indicatorsNumberChanged = true;
      }
      
      private function updateConfiguration() : void
      {
         if(this.configFlags != 0)
         {
            this.setupTexture();
            this.setupComponents();
         }
      }
      
      private function setupTexture() : void
      {
         var _local_1:int = 0;
         var _local_2:int = 0;
         this.size.setToZero();
         if(this.hasAnyFlag(TitleConfigFlags.LABEL))
         {
            this.createLabelComponents();
            this.label.text = this.labelText || "";
            this.resistanceLabel.color = MessageColor.YELLOW;
            this.resistanceLabel.filters = Filters.SHADOW_FILTERS;
            this.size.setWidth(PREMIUM_RANK_ICON_SIZE + LABEL_SPACING + this.label.textWidth + RESISTANCE_ICON_SPACING_X + RESISTANCE_ICON_WIDTH + RESISTANCE_MAX_TEXT_WIDTH);
            this.size.setHeight(LABEL_HEIGHT);
         }
         if(this.hasAnyFlag(TitleConfigFlags.HEALTH))
         {
            this.size.setWidthIfGreater(BAR_WIDTH);
            if(this.hasAnyFlag(TitleConfigFlags.LABEL))
            {
               this.size.addHeight(HEALTH_BAR_SPACING_Y);
            }
            this.size.addHeight(BAR_HEIGHT);
            if(this.hasAnyFlag(TitleConfigFlags.WEAPON))
            {
               this.size.addHeight(WEAPON_BAR_SPACING_Y + BAR_HEIGHT);
            }
         }
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS))
         {
            _local_1 = 4;
            _local_2 = _local_1 * EFFECTS_ICON_SIZE + (_local_1 - 1) * EFFECTS_SPACING_X;
            this.size.setWidthIfGreater(_local_2);
            if(this.hasAnyFlag(TitleConfigFlags.LABEL | TitleConfigFlags.HEALTH))
            {
               this.size.addHeight(EFFECTS_SPACING_Y);
            }
            this.size.addHeight(EFFECTS_ICON_SIZE);
         }
         this.size.addWidth(2 * TEXTURE_MARGIN);
         this.size.addHeight(2 * TEXTURE_MARGIN);
         this.createTexture();
         this.updateAdditionalUserTitle(this.healthBarSkin.color);
      }
      
      private function createTexture() : void
      {
         var _local_5:Boolean = false;
         var _local_1:int = Math.ceil(this.size.width);
         var _local_2:int = Math.ceil(this.size.height);
         var _local_3:int = MathUtils.nearestPowerOf2(this.size.width);
         var _local_4:int = MathUtils.nearestPowerOf2(this.size.height);
         if(this.texture == null || this.texture.width != _local_3 || this.texture.height != _local_4)
         {
            if(this.texture != null)
            {
               this.texture.dispose();
            }
            this.texture = new BitmapData(_local_3,_local_4,true,0);
            this.material.texture = this.texture;
            this.textureRect = this.texture.rect;
            _local_5 = true;
         }
         if(_local_5 || _local_1 != this.sprite.width || _local_2 != this.sprite.height)
         {
            this.sprite.width = _local_1;
            this.sprite.height = _local_2;
            this.sprite.bottomRightU = _local_1 / _local_3;
            this.sprite.bottomRightV = _local_2 / _local_4;
            this.invalidateAllConfigFlags();
         }
      }
      
      private function setupComponents() : void
      {
         var _local_1:int = 0;
         var _local_2:int = TEXTURE_MARGIN;
         if(this.hasAnyFlag(TitleConfigFlags.LABEL))
         {
            _local_2 += LABEL_HEIGHT;
         }
         if(this.hasAnyFlag(TitleConfigFlags.HEALTH))
         {
            if(this.hasAnyFlag(TitleConfigFlags.LABEL))
            {
               _local_2 += HEALTH_BAR_SPACING_Y;
            }
            _local_1 = this.size.width - BAR_WIDTH >> 1;
            this.healthBar = new ProgressBar(_local_1,_local_2,this.maxHealth,BAR_WIDTH,this.healthBarSkin);
            _local_2 += BAR_HEIGHT;
            if(this.hasAnyFlag(TitleConfigFlags.WEAPON))
            {
               _local_2 += WEAPON_BAR_SPACING_Y;
               this.weaponBar = new ProgressBar(_local_1,_local_2,100,BAR_WIDTH,ProgressBarSkin.WEAPONBAR);
               _local_2 += BAR_HEIGHT;
            }
         }
         if(this.hasAnyFlag(TitleConfigFlags.EFFECTS))
         {
            _local_2 += EFFECTS_SPACING_Y;
            this.effectIndicatorsY = _local_2;
            this.createEffectsIndicators();
         }
      }
      
      public function addToContainer() : void
      {
         this.container.addChild(this.sprite);
         this.time = getTimer();
      }
      
      public function removeFromContainer() : void
      {
         this.container.removeChild(this.sprite);
      }
      
      private function setPosition(_arg_1:Vector3) : void
      {
         this.sprite.x = _arg_1.x;
         this.sprite.y = _arg_1.y;
         this.sprite.z = _arg_1.z + this.zOffset;
         if(this.isLocal)
         {
         }
      }
      
      public function readPosition(_arg_1:Vector3D) : void
      {
         _arg_1.x = this.sprite.x;
         _arg_1.y = this.sprite.y;
         _arg_1.z = this.sprite.z;
      }
      
      public function setSuspicious(_arg_1:Boolean) : void
      {
         this.isSuspicious = _arg_1;
         this.invalidateAllConfigFlags();
      }
      
      private function invalidateConfigFlags(_arg_1:int) : void
      {
         this.dirtyFlags |= _arg_1 & this.configFlags;
      }
      
      public function hasAnyFlag(_arg_1:int) : Boolean
      {
         return (_arg_1 & this.configFlags) != 0;
      }
      
      private function createLabelComponents() : void
      {
         if(this.label == null)
         {
            this.label = new Label();
            this.label.gridFitType = GridFitType.PIXEL;
            this.label.autoSize = TextFieldAutoSize.LEFT;
            this.label.thickness = 200;
         }
      }
      
      private function updateLabel() : void
      {
         var _local_1:BitmapData = this.texture.clone();
         _local_1.fillRect(this.textureRect,0);
         var _local_2:int = this.getRankIconSize() + LABEL_SPACING + this.label.textWidth;
         var _local_3:* = this.size.width - _local_2 >> 1;
         matrix.tx = _local_3;
         matrix.ty = TEXTURE_MARGIN + this.getRankIconOffsetY();
         rankIcon.init(this.hasPremium,this.rankId);
         _local_1.draw(rankIcon,matrix,null,null,null,true);
         matrix.tx = _local_3 + this.getRankIconSize() + LABEL_SPACING;
         matrix.ty = TEXTURE_MARGIN;
         this.label.textColor = !this.isSuspicious ? uint(this.healthBarSkin.color) : uint(ColorConstants.SUSPICIOUS);
         _local_1.draw(this.label,matrix,null,null,null,true);
         this.texture.applyFilter(_local_1,this.textureRect,new Point(),Filters.SHADOW_FILTER);
         _local_1.dispose();
      }
      
      private function createEffectsIndicators() : void
      {
         var _local_1:int = 0;
         if(this.effectIndicators == null)
         {
            this.effectIndicators = new Vector.<EffectIndicator>();
            for each(_local_1 in inventoryItemTypes)
            {
               this.effectIndicators.push(new EffectIndicator(_local_1,this));
            }
         }
      }
      
      private function getEffectIndicatorById(_arg_1:int) : EffectIndicator
      {
         var _local_2:int = 0;
         var _local_3:int = 0;
         var _local_4:EffectIndicator = null;
         if(this.effectIndicators != null)
         {
            _local_2 = int(this.effectIndicators.length);
            _local_3 = 0;
            while(_local_3 < _local_2)
            {
               _local_4 = this.effectIndicators[_local_3];
               if(_local_4.effectId == _arg_1)
               {
                  return _local_4;
               }
               _local_3++;
            }
         }
         return null;
      }
      
      private function updateVisibility(_arg_1:int) : void
      {
         if(this.hidden)
         {
            if(this.sprite.alpha > 0)
            {
               this.sprite.alpha -= ALPHA_SPEED * _arg_1;
               if(this.sprite.alpha <= 0)
               {
                  this.sprite.alpha = 0;
                  this.sprite.visible = false;
               }
            }
         }
         else
         {
            this.sprite.visible = true;
            if(this.sprite.alpha < 1)
            {
               this.sprite.alpha += ALPHA_SPEED * _arg_1;
               if(this.sprite.alpha > 1)
               {
                  this.sprite.alpha = 1;
               }
            }
         }
      }
      
      private function getRankIconSize() : int
      {
         return !this.hasPremium ? int(RANK_ICON_SIZE) : int(PREMIUM_RANK_ICON_SIZE);
      }
      
      private function getRankIconOffsetY() : int
      {
         return !this.hasPremium ? int(RANK_ICON_OFFSET_Y) : int(PREMIUM_RANK_ICON_OFFSET_Y);
      }
      
      public function close() : void
      {
         if(this.material != null)
         {
            this.material.dispose();
            this.material = null;
         }
         if(this.texture != null)
         {
            this.texture.dispose();
            this.texture = null;
         }
         if(this.sprite != null)
         {
            this.sprite.material = null;
         }
         this.sprite = null;
         this.container = null;
         this.tankObject = null;
      }
   }
}

