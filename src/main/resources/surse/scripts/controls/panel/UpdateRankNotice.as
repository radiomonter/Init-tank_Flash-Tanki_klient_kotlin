package controls.panel
{
   ...
   public class UpdateRankNotice extends Sprite
   {
      ...
      public function UpdateRankNotice(_arg_1:int, _arg_2:int)
      {
         ...
         this.label.text = localeService.getText(TextConst.STRING_MAIN_PANEL_UPDATE_RANK_NOTICE,Rank.name(_arg_1));
         addChild(this.label);
         ...
         if(_arg_2 > 0)
         {
            ...
            this.gift.text = localeService.getText(TextConst.STRING_MAIN_PANEL_UPDATE_RANK_NOTICE_GIFT,_arg_2.toString());
            this.gift.x = -(this.gift.width >> 1);
            this.gift.y = GIFT_OFFSET;
            addChild(this.gift);
            _local_3 = new IconCrystalClass();
            _local_3.x = this.gift.x + this.gift.width + 2;
            _local_3.y = GIFT_OFFSET - 1;
            addChild(_local_3);
      ...
      private function stage_resizeHandler(_arg_1:Event) : void
      {
         x = stage.stageWidth / 2;
         y = stage.stageHeight - this.currentHeight >> 1;
      }
      
      private function timer_timerCompleteHandler(_arg_1:TimerEvent) : void
      {
      ...
      {
         var _local_2:Number = 1 - (getTimer() - this.beginHideTime) / this.hideTime;
         this.alpha = _local_2;
         if(_local_2 <= 0)
         {
